...
void find_cycle_recurs(size_t n, bitset start_vertex, bitset current_path, bool** path_matrix, size_t current_vertex, bitset* adj, int cycle_length, bitset** edges_found) {

    if (cycle_length == 0){
        if(intersection(adj[current_vertex], start_vertex)){
            for(int i = 0; i < n; i++){
                for(int j = 0; j < i; j++){
                    if(path_matrix[i][j] == true){
                        edges_found[i][j] |= singleton(size(current_path));
                    }
                }
            }
            return;
    ...
    forEach(vertex, vertices_to_check) {
        if(vertex  < current_vertex){
            path_matrix[current_vertex][vertex] = true;
        }else{
            path_matrix[vertex][current_vertex] = true;
        }
        
        find_cycle_recurs(n, start_vertex, current_path | singleton(vertex), path_matrix, vertex, adj, cycle_length -1, edges_found);
        if(vertex  < current_vertex){
            path_matrix[current_vertex][vertex] = false;
        }else{
            path_matrix[vertex][current_vertex] = false;
        }
    }
    return;
...
void find_cycle_of_given_length(bitset* adj, size_t n, int cycle_length, bitset** already_found) {
    bool** path_matrix = malloc(n * sizeof(bool*));
    bitset* edges_to_find = malloc(n * sizeof(bitset));

    for(int i = 0; i < n; i++){
        edges_to_find[i] = EMPTY;
        for(int j = 0; j <= i; j++){
            if(adj[i] & singleton(j)){
                if(intersection(already_found[i][j],singleton(cycle_length-1)) == 0){
                    edges_to_find[i] |= singleton(j);
                }
            }
        }
    }
    ...
    for(int i = 0; i < n; i++){
        path_matrix[i] = malloc(i+1 * sizeof(bool));
        for(int j = 0; j <= i; j++){
            path_matrix[i][j] = false;
        }
    }
    for(int i = 0; i < n; i++){
        forEach(j,edges_to_find[i]){
            if((already_found[i][j] & (1<<(cycle_length-1))) == 0){
                path_matrix[i][j] = true;
                find_cycle_recurs(n, singleton(j), singleton(i) & singleton(j), path_matrix, j, adj, cycle_length-1, already_found);
                path_matrix[i][j] = false;
        }
    }
...
void check_path_recurs(bitset first_vertex, bitset* adj, bitset* path, bool** path_matrix, bitset current_vertex, size_t n, bitset* end_points, bitset reachable_endpoints, size_t* counter, size_t* checked_via_degree, size_t* pruned_via_degree, bitset** cycles) {
    *counter += 1;
    ...
    if( (adj[current_vertex_integer] & first_vertex) > 0) {
        ...
        if(__builtin_ctzll(first_vertex) > current_vertex_integer){
            cycles[__builtin_ctzll(first_vertex)][current_vertex_integer] |= singleton(size(*path)-1);

        }
        else{
            cycles[current_vertex_integer][__builtin_ctzll(first_vertex)] |= singleton(size(*path)-1);
        }

        for(int i = 0; i < n; i++){
            for(int j = 0; j < i; j++){
                if(path_matrix[i][j] == true){
                    cycles[i][j] |= singleton(size(*path)-1);
                }
            }
...
bool check_ham_conn_graph6(const char* graph_str, size_t* counter, size_t* checked_via_degree, size_t* pruned_via_degree, char* filename, char* interesting_criteria) {
    ...
    */
    ...
    if(strcmp(interesting_criteria, "all") == 0){
        ...
        for(int i = 0; i < n; i++){
           forEach(j, adj[i]){
            if (i > j){
                if(cycles[i][j] != complement(EMPTY,n-1)){
                    find_cycle_of_given_length(adj,n,i+1, cycles);
                }
            }
        }
    }
        ...
        for(int i = 0; i < n; i++){
            forEach(j, adj[i]){
                ...
                for(int k = 0; k < n; k++){
                    fprintf(fptr, "%c", cycles[i][j] & (1 << k) ? '1' : '0');
                }
    ...
    if(strcmp(interesting_criteria, "last") == 0){
        ...
        for(int i = 0; i < n; i++){
            forEach(j, adj[i]){
                if (j < i){
                    if((cycles[i][j] & (1<<(n-2)))==0){
                        fprintf(stderr,"cycle: ");
                        for(int k = 0; k < n-1; k++){
                            fprintf(stderr,"%c", cycles[i][j] & (1 << k) ? '1' : '0');
                        }
                        fprintf(stderr,"\n");
                        fptr = fopen(filename, "a");
                        fprintf(fptr, "%i,%i ", i, j);
                        is_interesting = true;
                    }
                }     
            }
